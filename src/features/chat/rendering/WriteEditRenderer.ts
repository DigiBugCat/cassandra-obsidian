...
import type { WriteEditState } from '../state';
import { setupCollapsible } from './collapsible';
import { renderDiffContent } from './DiffRenderer';
import { applyFileLink, fileNameOnly } from './ToolCallRenderer';
...
export function createWriteEditBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'cassandra-write-edit-block' });
  wrapperEl.dataset.toolId = toolCall.id;

  // Header (clickable to collapse/expand)
  const headerEl = wrapperEl.createDiv({ cls: 'cassandra-write-edit-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');
  headerEl.setAttribute('aria-label', `${toolName}: ${shortenPath(filePath)} - click to expand`);

  // File icon
  const iconEl = headerEl.createDiv({ cls: 'cassandra-write-edit-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(toolName));

  const nameEl = headerEl.createDiv({ cls: 'cassandra-write-edit-name' });
  nameEl.setText(toolName);
  const summaryEl = headerEl.createDiv({ cls: 'cassandra-write-edit-summary' });
  ...
  const statsEl = headerEl.createDiv({ cls: 'cassandra-write-edit-stats' });

  const statusEl = headerEl.createDiv({ cls: 'cassandra-write-edit-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  // Content area (collapsed by default)
  const contentEl = wrapperEl.createDiv({ cls: 'cassandra-write-edit-content' });

  // Initial loading state
  const loadingRow = contentEl.createDiv({ cls: 'cassandra-write-edit-diff-row' });
  const loadingEl = loadingRow.createDiv({ cls: 'cassandra-write-edit-loading' });
...
export function updateWriteEditWithDiff(state: WriteEditState, diffData: ToolDiffData): void {
  ...
  // Render diff content
  const row = state.contentEl.createDiv({ cls: 'cassandra-write-edit-diff-row' });
  const diffEl = row.createDiv({ cls: 'cassandra-write-edit-diff' });
...
export function finalizeWriteEditBlock(state: WriteEditState, isError: boolean): void {
  ...
  state.statusEl.className = 'cassandra-write-edit-status';
  ...
  if (isError) {
    ...
    if (!state.diffLines) {
      state.contentEl.empty();
      const row = state.contentEl.createDiv({ cls: 'cassandra-write-edit-diff-row' });
      const errorEl = row.createDiv({ cls: 'cassandra-write-edit-error' });
    ...
    state.contentEl.empty();
    const row = state.contentEl.createDiv({ cls: 'cassandra-write-edit-diff-row' });
    const doneEl = row.createDiv({ cls: 'cassandra-write-edit-done-text' });
    doneEl.setText('DONE');
  }
...
export function renderStoredWriteEdit(parentEl: HTMLElement, toolCall: ToolCallInfo): HTMLElement {
  ...
  const statusEl = headerEl.createDiv({ cls: 'cassandra-write-edit-status' });
  if (isError) {
  ...
  const row = contentEl.createDiv({ cls: 'cassandra-write-edit-diff-row' });
