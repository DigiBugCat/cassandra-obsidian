import type { ThinkingBlockState } from '../state';
...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'cassandra-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'cassandra-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'cassandra-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'cassandra-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.cassandra-thinking-header');
  if (header) {
